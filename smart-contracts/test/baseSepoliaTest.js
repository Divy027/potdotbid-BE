...
async function getBalance(address) {
    ...
    let b = await ethers.provider.getBalance(address);
...
// base sepolia
// const routerV2Addr = "0x1689E7B1F10000AE47eBfE339a4f69dECd19F602"
// const factoryAddr = "0x7Ae58f10f7849cA6F5fB71b7f45CB416c9204b1e"
// const weth = "0x4200000000000000000000000000000000000006"; // done 
// const linkAddr = "0xE4aB69C077896252FAFBD49EFD26B5D171A32410"; // done

// etherehum
const routerV2Addr = "0x1689E7B1F10000AE47eBfE339a4f69dECd19F602"
const factoryAddr = "0x7Ae58f10f7849cA6F5fB71b7f45CB416c9204b1e"
const weth = "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2"; // done 
const linkAddr = "0x514910771AF9Ca656af840dff83E8264EcF986CA"; // done
...
describe("Lock", function () {
    ...
    async function deployOneYearLockFixture() {
        ...
        const [owner, otherAccount, feeReciver] = await ethers.getSigners();

        const maxPurchaseAmount = ethers.parseEther("0.0000001");
        const BondingCurve = await ethers.getContractFactory("TestBondingCurve");
        ...
        const router = new ethers.Contract(routerV2Addr, routerABI, owner)
    ...
    describe("Deployment", function () {
        it("buy and sell", async function () {
            ...
            const { bondingCurve, owner, router, feeReciver, otherAccount } = await deployOneYearLockFixture();

            // create token
            let tx = await bondingCurve.createAndInitPurchase("divy", "ddd", {
            ...
            // const _ethAmont = await bondingCurve.getEthAmountBySale(tokenAddr, "10000000000000");
            // tx = await bondingCurve.sellToken(tokenAddr, "10000000000000", _ethAmont);
            // await tx.wait()
            // console.log("completed...")

            // console.log(`Feee reciver balance after selling the token (sent) ${ethers.formatEther(_ethAmont)} : ${await getBalance(feeReciver.address)}`)

    

            // send some extra eth
            console.log("bying tokens...")
            // ethAmount = ethers.parseEther("0.00009")
            const tTotal = (await bondingCurve.virtualPools(tokenAddr))[1]
            let ethAmount = await bondingCurve.getEthAmountToBuyTokens(tokenAddr, tTotal.toString());
            console.log("eth amount : ", ethAmount.toString())
            const tokensWillRecive = await bondingCurve.getTokenAmountByPurchase(tokenAddr, ethAmount);

            // const c = (Number(tTotal) - Number(tokensWillRecive)) / Number(tokensWillRecive)

            // console.log("diffence : ", c)


            // console.log("tokenswill recive : ", tokensWillRecive.toString()) 
            // console.log("totaol token :", tTotal)

            ethAmount = BigInt(ethAmount) + BigInt(ethers.parseEther('0.0001')) // sseidnig extra to active pool
            tx = await bondingCurve.purchaseToken(tokenAddr, tokensWillRecive, {
                value: ethAmount
            });
            await tx.wait()
            ...
            // ethAmount = ethers.parseUnits("0.000001", "ether"); // 0.1 ETH
            // const estimatedTokenAmount = await router.getAmountsOut(ethAmount, [weth, tokenAddr]);
            // console.log("Estimated Token Amount: ", estimatedTokenAmount[1].toString());

            // await router.connect(otherAccount).swapExactETHForTokensSupportingFeeOnTransferTokens(
            //     estimatedTokenAmount[1], // Minimum amount of tokens to receive
            //     [weth, tokenAddr],          // Swap path
            ...
            // let linkBlanceOftokenContract = await linkToken.balanceOf(tokenAddr);
            // console.log("link balance of token (before register upkeep) : ", linkBlanceOftokenContract.toString(), tokenAddr)
            ...
            //     amount: ethers.parseEther("1"), // uint96
            // };
            // tx = await _token.registerAndPredictID(registrationParams);
            // await tx.wait()

            // linkBlanceOftokenContract = await linkToken.balanceOf(tokenAddr);
            // console.log("link balance of token (after register upkeep) : ", linkBlanceOftokenContract.toString(), tokenAddr)
